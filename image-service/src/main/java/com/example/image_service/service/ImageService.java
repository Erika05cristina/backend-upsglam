...
public class ImageService {
    ...
    public Mono<AvatarUploadResponse> uploadAvatar(MultipartFile file, String userId) {
        ...
        if (file.getSize() > 5 * 1024 * 1024) {
            return Mono.error(new RuntimeException("El archivo excede el límite de 5MB"));
        }

        // Validación tipo MIME
        String contentType = file.getContentType();
        if (!"image/png".equals(contentType) && !"image/jpeg".equals(contentType)) {
            return Mono.error(new RuntimeException("Solo se permiten PNG o JPG"));
        }
        ...
        return Mono.fromCallable(file::getBytes)
                .flatMap(bytes -> {
                    ...
                    return uploadBytes(bytes, fileName)
                            .map(url -> new AvatarUploadResponse(url));
                });
    }
}
